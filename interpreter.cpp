...
int runinterpreter(char inputtext[TEXT_BUFFER][TEXT_BUFFER])
{

	int codeSize = 0;
	char input[TEXT_BUFFER][TEXT_BUFFER];

	memset(input,0,sizeof(char)*TEXT_BUFFER*TEXT_BUFFER);
	for(int i=0, j=0;i<TEXT_BUFFER;i++){
		if (inputtext[i]!=0){
			memcpy(input[j],inputtext[i],sizeof(char)*TEXT_BUFFER);
			codeSize++;
			j++;
		}
	}
